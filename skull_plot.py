...
def do_skull_plot(vec20, fname, dataset):
    ...
    data = _N.zeros((1, n_channels, sfreq * 1))
    #X =X(:,[1:8 10:21]);
...
"""
"""
def do_skull_plot_all_EVs(all_vecs, ps, ch_names, fname, eeg_date, fL, fH, montage="standard_1020", close=False, extra_suptitle=None):
    # Create some dummy metadata
...
#                            sphere=(0., 0., 0., 0.09))
    ...
    suptitle = "%(fn)s   [%(fL)d-%(fH)d]Hz" % {"fn" : eeg_date, "fL" : fL, "fH" : fH}
    if extra_suptitle is not None:
        suptitle = "%(st)s  %(est)s" % {"st" : suptitle, "est" : extra_suptitle}
...
def do_skull_plot_all_EVs_timeseries(all_vecs, ps, ch_names, fname, eeg_date, fL, fH, montage="standard_1020", close=False, extra_suptitle=None):
    # Create some dummy metadata
    epochs = all_vecs.shape[0]  #  all_vecs epochs x nChs x nStates
    n_channels = len(ch_names)
...
#              "eeg", "eeg", "eeg", "eeg", "eeg"]
    ...
    nEVs = all_vecs.shape[2]
    print(nEVs)
    print(all_vecs.shape)

    data = _N.zeros((n_channels, epochs*sfreq * nEVs))
    #data = _N.zeros((epochs, n_channels, sfreq * nEVs))
    print(data.shape)
    #X =X(:,[1:8 10:21]);
    for epc in range(epochs):
        for ns in range(nEVs):
            ...
            # data[0, 8, ns]   = 0.5*(all_vec20[0, ns] + all_vec20[6, ns])
            if montage == "standard_1020":
                data[0:n_channels, epc*nEVs + ns] = all_vecs[epc, 0:n_channels, ns]
                #data[epc, 0:n_channels, ns] = all_vecs[epc, 0:n_channels, ns]
            else:
                data[epc, 0:n_channels, ns] = all_vecs[epc, 0:n_channels, ns]
        if montage != "standard_1020":
            data[0, n_channels-1, ns] = _N.mean(all_vecs[:, ns], axis=0)
    ...
    #data[0, :, nEVs-1] = 0
    #evoked_array = mne.EvokedArray(data[0], info, tmin)
    evoked_array = mne.EvokedArray(data, info, tmin)

    all_evs = _N.arange(epochs*nEVs)


    fig, axes = _plt.subplots(figsize=(1.9*nEVs, 2.5*epochs), nrows=epochs, ncols=nEVs)
    ...
    for ev in range(nEVs):
        if nEVs > 1:
            ...
            ax = axes
        evoked_array.plot_topomap(all_evs[ev], ch_type="eeg", axes=axes, show=False, colorbar=False, nrows=epochs, ncols=nEVs)
#0.1, ch_type=ch_type, size=2, extrapolate=extr,
#                            axes=ax, show=False, colorbar=False,
#                            sphere=(0., 0., 0., 0.09))
        ax.set_title('ev %(ev1)d[%(p)d]' % {"ev1" : (ev+1), "p" : ps[ev]})
